        ...
        while queue:
            for _ in range(len(queue)):
                ...
                if combination == target:
                    return res
                for i in range(4):
                    plusOneCombination = combination[0:i]+str(((int(combination[i])+1)+10)%10)+combination[i+1:]
                    minusOneCombination = combination[0:i]+str(((int(combination[i])-1)+10)%10)+combination[i+1:]
